...
def parse_mqm_answer(x, count_limit=False):
    if x is None or not isinstance(x, str):
        return None, None
    x = x.lower()
    x = x.replace('**', '').replace("- ", "")
    x = x.replace("critical errors:", "critical:").replace("major errors:", "major:").replace("minor errors:", "minor:")
    x = x.replace("critical error:", "critical:").replace("major error:", "major:").replace("minor error:", "minor:")
    x = x.replace("critical: ", "critical:\n").replace("major: ", "major:\n").replace("minor: ", "minor:\n")
    ...
    for i, line in enumerate(x.split('\n')):
        ...
        if "no-error" in line or "no error" in line or "" == line or '':
            continue
        if "critical:" == line:
        ...
        #else:
        if not any([w in line for w in ['none', 'no-error', 'no error', 'no issue', 'no issues']]) and lines[i-1]!='':
            if any([line.startswith(x) for x in ['accuracy', 'fluency', 'locale convention', 'style', 'terminology', 'non-translation', 'other']]):
            #f line == 'no minor errors in this case.':
                #print(">>>")
                #print(lines[-2:])
            #    raise ValueError
    ...
    return x, errors, -final_score


def mqm_fewshot(few_shots):
    prompts = [
        # No System Prompt for Deepseek
        #{
        #    "role": "system",
...
Here is an example for you; please strictly follow the answer format:
"""
